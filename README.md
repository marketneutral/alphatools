...
* `rank(x)`: ranks, per day, across all assets (i.e., the cross-sectional rank); ranks are descending such that the rank of the maximum raw value in the vector is N, where N is the number of assets; the smallest rank is 1. The ordinal method is used to match `Pipeline` method `.rank()`.
...
* `ts_argmax(x, days)`: on which day `ts_max(x, days)` occurred (also `ts_argmin(...)`). For example, if window (*days*) is 10 days, and the max is in the most recent day, it will return 10; If the max is in the earliest day it will return 1.
