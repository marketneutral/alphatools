...
* `rank(x)`: scaled ranks, per day, across all assets (i.e., the cross-sectional rank); ranks are descending such that the rank of the maximum raw value in the vector is 1.0; the smallest rank is 1/N. The re-scale of the ranks to the interval (0,1] is implied by Alpha 1: 0.50 is subtracted from the final ranked value. The ordinal method is used to match `Pipeline` method `.rank()`.
...
* `ts_argmax(x, days)`: on which day `ts_max(x, days)` occurred (also `ts_argmin(...)`) scaled to the interval (0,1]. For example, if window (*days*) is 10 days, and the max is in the most recent day, it will return 1.0; if the max is in the earliest day it will return 0.10.
* `ts_rank(x, days)`: the time series rank per asset on *x* over the the trailing *days*. Currently this is in the range [0,1], but should be (0,1].
